            ...
            <div class="jumbotron jumbotron-fluid">
                <div class="container table-responsive-lg table-responsive-md table-responsive-xl">
                    <table class="table">
                        ...
                        <tr>
                            <td>{{ $oneNews->rubric }}</td>
                            <td>{{ $oneNews->title }}</td>

                            @if ($oneNews->created_at)
                                <td>{{ date("d.m.Y", strtotime($oneNews->created_at)) }}</td>
                            @else
                                <td>{{ date('d.m.Y', strtotime($oneNews->updated_at)) }}</td>
                            @endif
                        
                            <td>{!! strip_tags(str_limit($oneNews->description, 200)) !!}</td>
                            <td>{{ $oneNews->url }}</td>
                            <td>
                                <form action="{{ url('/admin/news/'. $oneNews->id . '/edit') }}" method="POST">
                                    @csrf
                                    <button type="submit" class="btn btn-outline-success btn-sm btn-block">Редактировать</button>
                                </form>
                                
                                <form action="{{ url('/admin/news/' . $oneNews->id) }}" method="POST">
                                    @csrf
                                    {{ method_field('DELETE') }}
                    ...
                    <form action="news" method="post">
                        @csrf
                        <table class="table">
                            <h3 class="text-center text-success">Добавить новость</h3>
                            <tr>
                                ...
                                <td><button type="submit" class="btn btn-outline-success btn-sm btn-block">Добавить новость</button></td>
                            </tr>
                        </table>
                    </form>
